...
const paper = { source: "MED", pmcid: "PMC12345", doi: "10.1234/example", title: "Example paper", abstractText: "Abstract", license: "CC BY 4.0", firstPublicationDate: "2025-06-01", citedByCount: 17, authorList: { author: [{ fullName: "A. Author" }] }, pubTypeList: { pubType: ["Journal Article"] } };
...
describe("additional research indexes", () => {
    it("maps index provenance separately from the content host and respects strict licensing", () => {
        ...
        expect(candidate).toMatchObject({ database: "nih", paperId: "12345", doi: "10.1234/example", indexedBy: ["Europe PMC"], citationCount: 17, citationSource: "europepmc", access: { canSendToAI: true } });
    ...
    it("only admits exact Crossref DOI matches resolved to readable PMC candidates", async () => {
        const fetch = vi.fn().mockResolvedValueOnce(Response.json({ message: { items: [{ DOI: "https://doi.org/10.1234/example", "is-referenced-by-count": 41 }] } }))
        ...
        expect(result.candidates[0].citationCount).toBe(41);
        expect(result.candidates[0].citationSource).toBe("crossref");
