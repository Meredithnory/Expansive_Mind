...
- Discover share: **Share synthesis** (signed-in only, `result.id` is a 24-char hex ObjectId). Enabled only when every claim-ledger row has a non-empty OA full-text excerpt, a resolvable paper citation (DOI, paper id, or href), and a commercial-friendly license URI (CC0, CC BY, CC BY-SA, or CC BY-ND). Scholar snippets and abstracts are not quote sources. Incomplete: the button stays visible and disabled; `#discover-share-lock` explains why. Success label **Link copied!**.
